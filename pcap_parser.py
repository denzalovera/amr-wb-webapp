...
def storePayloadIetf(outfile, codec, payload):
    ...
    if codec == 'amr' or codec == 'amr-wb':
        ...
        # Validate frame type is within expected range
        if ft >= len(codec_ft):
            logging.warning(f'Invalid frame type {ft} for codec {codec}, skipping packet')
            num_bad_frames += 1
            return
...
def storePayloadIu(outfile, codec, amrpl):
    ...
    if isvalid == True:
        ...
        # Validate frame type index exists in mapping
        if ft_index not in codec_ft_map:
            logging.warning(f'Invalid frame size {ft_index} for codec {codec}, skipping packet')
            num_bad_frames += 1
            return
        ...
        # Validate frame type is within expected range
        if ft >= len(codec_ft):
            logging.warning(f'Invalid frame type {ft} for codec {codec} in Iu framing, skipping packet')
            num_bad_frames += 1
            return
